...
const Edit_RJ_MUTATION = gql`
  mutation EditPhotoRJ(
    $id: Int!
    $file: Upload
    $file2: Upload
    $file3: Upload
...
function EditPhotoPoliosRJ(props: any) {
  const { register, handleSubmit, setValue, getValues } = useForm<any>({
    mode: 'onChange',
  })
  const pInfo = props.location.state.pInfoRJ
  ...
  const [editPhotoRJMutaion] = useMutation<any>(Edit_RJ_MUTATION, {
    onCompleted,
  })
...
export default EditPhotoPoliosRJ
